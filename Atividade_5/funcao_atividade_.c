...
void tratar_eventos_leds() {
    ...
    while (true) {
        ...
        if (!gpio_get(BOTOES[id1])) {
            ...
            if (id1 == 0) {  // BOTÃO A → incrementa
                if (index_neo < LED_COUNT) {
            ...
            else if (id1 == 1) {   // BOTÃO B → decrementa
                if (index_neo > 0) {
                    index_neo--;
            ...
            else if (id1 == 2) {   // BOTÃO JOYSTICK → reset
                // Zera o contador e apaga a matriz
                contador = 0;
                index_neo = 0;
                npClear();
                npWrite();
                ...
                printf("Sistema resetado! Fila zerada.\n");
            }
